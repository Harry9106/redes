...
class StateTransitionDiagram:
    ...
    def transition(self, src, dst, flags):
        ...
        if dst == self.address:
            ...
            elif self.state == TCPState.synSent:
            	if "S" in flags and "A" in flags:
					new_state = TCPState.established
            elif self.state == TCPState.synReceived:
				if "A" in flags:
					new_state = TCPState.established
...
class Reassembler:
    ...
    def get_payload(self):
        ...
        """
        sorted_dic = sorted(self.dictionary.items(), key=operator.itemgetter(0))
        self.payload = array.array('B')
        for a,b in sorted_dic:
            self.payload.fromstring(b["Raw"].load)
        return self.payload
